...
# Non-health paths: alphanumeric segments separated by slashes
...
class TestMetricsMiddlewareStructuredLogs(SimpleTestCase):
    ...
    **Validates: Requirements 2.1, 2.5**
    ...
    def test_emits_complete_structured_log(self, method, path, status_code):
        ...
        # Exactly one metric log should have been emitted
        metric_records = [
            r for r in handler.records
            if getattr(r, "extra", None) and r.extra.get("type") == "request_metric"
        ]
        self.assertEqual(len(metric_records), 1, f"Expected 1 metric log, got {len(metric_records)}")

        extra = metric_records[0].extra
        required_keys = {"type", "method", "path", "status_code", "duration_ms", "request_id"}
        self.assertTrue(
            required_keys.issubset(extra.keys()),
            f"Missing keys: {required_keys - extra.keys()}",
        )
        self.assertEqual(extra["type"], "request_metric")
        self.assertEqual(extra["method"], method)
        self.assertEqual(extra["path"], path)
        self.assertEqual(extra["status_code"], status_code)
        self.assertEqual(extra["request_id"], "test-req-id")
...
class TestMetricsMiddlewareSkipsHealthPaths(SimpleTestCase):
    ...
    **Validates: Requirements 2.3**
    ...
    def test_no_metric_log_for_health_paths(self, method, path):
        ...
        # No metric logs should be emitted
        metric_records = [
            r for r in handler.records
            if getattr(r, "extra", None) and r.extra.get("type") == "request_metric"
        ]
...
class TestMetricsMiddlewareDurationPositive(SimpleTestCase):
    ...
    **Validates: Requirements 2.2**
    ...
    def test_duration_is_non_negative(self, method, path, status_code):
        ...
        duration_ms = metric_records[0].extra["duration_ms"]
