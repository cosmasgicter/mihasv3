...
notification_types = st.sampled_from([
    "deadline_warning", "deadline_urgent", "incomplete_draft_reminder",
    "missing_document_reminder", "application_update", "payment_update",
])

app_ids = st.integers(min_value=1, max_value=1_000_000)

decision_statuses = st.sampled_from(["approved", "rejected"])
...
class TestStatusChangeEventDispatch(SimpleTestCase):
    """# Feature: admissions-logic-canonicalization, Property 22: Status change event dispatch

    For any Application that transitions to `approved` or `rejected`, the
    `dispatch_event()` function SHALL be called with
    `event_type="application_update"` and the Application owner's user ID.

    **Validates: Requirements 10.3, 10.6**
    """

    @given(
        new_status=decision_statuses,
        notes=st.text(
            alphabet=st.sampled_from("abcdefghijklmnopqrstuvwxyz "),
            min_size=0,
            max_size=50,
    ...
    @settings(max_examples=100)
    def test_approved_rejected_dispatches_application_update_event(
        self, new_status, notes
    ):
        """When ApplicationReviewView transitions to approved/rejected,
        dispatch_event is called with event_type='application_update'."""
        ...
        mock_request.data = {"new_status": new_status, "notes": notes}

        with patch(
            "apps.applications.views.Application.objects"
        ) as mock_app_qs, patch(
            "apps.applications.views.dispatch_event"
        ) as mock_dispatch, patch(
            "apps.applications.views.transition_application_status",
            return_value="under_review",
        ), patch(
            "apps.applications.views.ApplicationReviewSerializer"
        ) as mock_serializer_cls, patch(
            "apps.applications.views.ApplicationStatusHistory"
        ):
            mock_app_qs.get.return_value = mock_app

            # Make the serializer valid and return the right data
            ...
            # If approving, mock payment check
            if new_status == "approved":
                mock_app.payment_status = "verified"

            view = ApplicationReviewView()
            view.post(mock_request, app_id)

            # dispatch_event MUST have been called with application_update
            mock_dispatch.assert_called()
            found_application_update = False
            for call in mock_dispatch.call_args_list:
                kwargs = call.kwargs
                if kwargs.get("event_type") == "application_update":
                    found_application_update = True
                    self.assertEqual(kwargs["user_id"], user_id)
                    self.assertIn("application_id", kwargs["payload"])
                    self.assertEqual(kwargs["payload"]["status"], new_status)
                    break
            self.assertTrue(
                found_application_update,
                f"dispatch_event was not called with event_type='application_update' for status={new_status}",
            )
...
class TestNotificationDeduplication(SimpleTestCase):
    """# Feature: admissions-logic-canonicalization, Property 23: Notification deduplication

    For any notification type and Application, the notification automation
    service SHALL dispatch at most one SSE event per 24-hour period — a
    second dispatch within the same window SHALL be a no-op.

    **Validates: Requirements 10.8**
    """

    @given(
        notif_type=notification_types,
        app_id=app_ids,
    )
    @settings(max_examples=100)
    def test_first_call_returns_true_second_returns_false(
        self, notif_type, app_id
    ):
        """_should_send() returns True on first call, False on second call
        for the same notification type and application ID."""
        from apps.applications.notification_tasks import _should_send, _dedup_key

        expected_key = _dedup_key(notif_type, app_id)

        with patch("apps.applications.notification_tasks.cache") as mock_cache:
            # First call: cache.add returns True (key did not exist)
            mock_cache.add.return_value = True
            result_first = _should_send(notif_type, app_id)
            self.assertTrue(
                result_first,
                "First _should_send() call should return True (notification not yet sent)",
            )
            mock_cache.add.assert_called_with(expected_key, 1, 24 * 60 * 60)

            # Second call: cache.add returns False (key already exists)
            mock_cache.add.return_value = False
            result_second = _should_send(notif_type, app_id)
            self.assertFalse(
                result_second,
                "Second _should_send() call should return False (deduplicated)",
            )

    @given(
        notif_type_a=notification_types,
        notif_type_b=notification_types,
        app_id=app_ids,
    )
    @settings(max_examples=100)
    def test_different_types_are_independent(
        self, notif_type_a, notif_type_b, app_id
    ):
        """Different notification types for the same application use
        independent deduplication keys."""
        from apps.applications.notification_tasks import _dedup_key

        key_a = _dedup_key(notif_type_a, app_id)
        key_b = _dedup_key(notif_type_b, app_id)

        if notif_type_a != notif_type_b:
            self.assertNotEqual(
                key_a,
                key_b,
                "Different notification types must produce different dedup keys",
            )
        else:
            self.assertEqual(
                key_a,
                key_b,
                "Same notification type must produce the same dedup key",
            )
    ...
    @settings(max_examples=100)
    def test_different_apps_are_independent(
        self, notif_type, app_id_a, app_id_b
    ):
        """Same notification type for different applications use
        independent deduplication keys."""
        from apps.applications.notification_tasks import _dedup_key

        key_a = _dedup_key(notif_type, app_id_a)
        key_b = _dedup_key(notif_type, app_id_b)

        if app_id_a != app_id_b:
            self.assertNotEqual(
                key_a,
                key_b,
                "Different app IDs must produce different dedup keys",
            )
        else:
            self.assertEqual(
                key_a,
                key_b,
                "Same app ID must produce the same dedup key",
            )
