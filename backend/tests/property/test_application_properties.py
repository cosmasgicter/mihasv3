    ...
    **Validates: Requirements 4.7**
    ...
    def test_non_approval_status_skips_payment_check(self, new_status):
        ...
        with patch("apps.applications.admin_review_views.transaction.atomic"), \
            ...
            with patch("apps.applications.admin_review_views.ApplicationStatusHistory.objects") as mock_history:
                mock_history.create.return_value = MagicMock()

                with patch("apps.applications.admin_review_views.submit_application", return_value=(app, "submitted")):
                    view = ApplicationReviewView()
                    response = view.post(request, application_id=app.id)
...
class TestDraftAutoSaveRoundTrip(SimpleTestCase):
    ...
    **Validates: Requirements 4.9**
    ...
    def test_draft_save_and_load_round_trip(self, draft_data):
        ...
        with patch("apps.applications.student_draft_views.ApplicationDraft.objects") as mock_qs:
