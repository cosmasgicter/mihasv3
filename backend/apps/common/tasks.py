...
SSE_CLEANUP_BATCH_SIZE = 1000


@shared_task(bind=True, max_retries=1, default_retry_delay=300)
def cleanup_sse_events_task(self):
    """Purge stale SSE event records in batches of 1000.

    Deletes delivered events where delivered_at is older than 7 days,
    and undelivered events where created_at is older than 7 days.
    Batches deletes to avoid long-running transactions.
    ...
    cutoff = timezone.now() - timedelta(days=SSE_EVENT_RETENTION_DAYS)
    total_deleted = 0

    try:
        # Delete delivered events older than 7 days.
        delivered_deleted = 0
        while True:
            batch_ids = list(
                ...
                .values_list("id", flat=True)[:SSE_CLEANUP_BATCH_SIZE]
            )
            if not batch_ids:
                break
            deleted_count, _ = SSEEvent.objects.filter(id__in=batch_ids).delete()
            delivered_deleted += deleted_count

        # Delete undelivered events older than 7 days.
        ...
        while True:
            ...
            if not batch_ids:
                break
            deleted_count, _ = SSEEvent.objects.filter(id__in=batch_ids).delete()
            undelivered_deleted += deleted_count

        total_deleted = delivered_deleted + undelivered_deleted
        logger.info(
            "SSE event cleanup: deleted %d delivered and %d undelivered events older than %s",
            delivered_deleted,
            undelivered_deleted,
            cutoff.isoformat(),
        )
        ...
        logger.error("SSE event cleanup failed: %s", exc)
...
def keep_alive_ping_task():
    ...
    import requests as http_requests
    try:
        url = os.environ.get("HEALTH_CHECK_URL", "***REMOVED***/health/live/")
