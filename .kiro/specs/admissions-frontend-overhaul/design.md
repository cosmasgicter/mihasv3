...
VITE_API_BASE_URL=https://api.mihas.edu.zm  # Production default
...
*For any* value of `VITE_API_BASE_URL`, `getApiBaseUrl()` shall return that value with trailing slashes and `/api/v1` suffixes stripped. When `VITE_API_BASE_URL` is not set and the browser origin is `https://apply.mihas.edu.zm`, the function shall return `https://api.mihas.edu.zm`. When neither is available, it shall default to `https://api.mihas.edu.zm`.
