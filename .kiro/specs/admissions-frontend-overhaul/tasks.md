...
- [ ] 1. API Client Core Migration
  - [ ] 1.1 Rewrite `src/services/client.ts` — remove `normalizeEndpoint()` and `supportedResources`
  ...
  - [ ] 1.2 Update `request()` to enforce `credentials: 'include'` and CSRF handling
  ...
  - [ ] 1.3 Update auth-related methods in `client.ts` to use Django REST paths
  ...
  - [ ] 1.4 Update timeout patterns and query invalidation in `client.ts`
  ...
  - [ ] 1.5 Update `unwrapApiResponse()` and error parsing
  ...
  - [ ] 1.6 Add `VITE_API_BASE_URL` environment variable support
    - Ensure `getApiBaseUrl()` reads `VITE_API_BASE_URL`, strips trailing slashes and `/api/v1` suffixes
    - Default to `https://api.mihas.edu.zm` when the variable is not set
  ...
  - [ ]* 1.7 Write property tests for API client core (Properties 1–7, 11–13)
...
- [ ] 2. Checkpoint — API client core complete
...
- [ ] 3. Service Layer Migration
  - [ ] 3.1 Migrate `src/services/auth.ts` to Django REST paths
  ...
  - [ ] 3.2 Migrate `src/services/applications.ts` to Django REST paths
  ...
  - [ ] 3.3 Migrate `src/services/catalog.ts` to Django REST paths
  ...
  - [ ] 3.4 Migrate `src/services/documents.ts` and `src/services/documentExtraction.ts`
  ...
  - [ ] 3.5 Migrate `src/services/notifications.ts`
  ...
  - [ ] 3.6 Migrate `src/services/sessionService.ts`
