...
1. THE API_Client SHALL read the backend base URL from the `VITE_API_BASE_URL` environment variable (defaulting to `https://api.mihas.edu.zm`) and use it for all outbound requests
...
2. THE Admissions_App SHALL default `VITE_API_BASE_URL` to `https://api.mihas.edu.zm` in production when the variable is not set
