...
- [x] 1. Extract ButtonSpinner and create skeleton page components
  - [x] 1.1 Create `ButtonSpinner` component at `src/components/ui/ButtonSpinner.tsx`
  ...
  - [x] 1.2 Update `Button.tsx` to import from `ButtonSpinner` instead of `UnifiedLoader`
  ...
  - [x] 1.3 Add `DashboardSkeleton`, `AuthSkeleton`, and `WizardSkeleton` to `src/components/ui/skeleton.tsx`
  ...
  - [x] 1.4 Write property test for skeleton type mapping (Property 11)
  ...
  - [x] 1.5 Wire skeleton fallbacks into route Suspense boundaries
...
- [x] 2. Checkpoint — Verify ButtonSpinner and skeletons
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Create `useStyleInjection` hook and migrate SmoothUI components
  - [x] 3.1 Create `src/hooks/useStyleInjection.ts`
  ...
  - [x] 3.2 Write property test for style deduplication (Property 7)
  ...
  - [x] 3.3 Migrate SmoothUI components to use `useStyleInjection`
...
- [x] 4. Replace UnifiedLoader imports — Auth pages and shared UI components
  - [x] 4.1 Migrate auth pages: `SignInPage.tsx`, `SignUpPage.tsx`, `AuthCallbackPage.tsx`
    - Remove `UnifiedLoader` imports, replace overlay loading states with `AuthSkeleton` or remove
    - _Requirements: 1.2, 1b.2, 1b.5_

  - [x] 4.2 Migrate shared UI wrapper components
...
- [x] 5. Replace UnifiedLoader imports — Wizard and student pages
  - [x] 5.1 Migrate wizard: `applicationWizard/index.tsx`, `SubmissionSuccess.tsx`
  ...
  - [x] 5.2 Migrate student pages: `NotificationSettings.tsx`, `ApplicationStatus.tsx`, `Payment.tsx`, `Interview.tsx`, `Settings.tsx`, `ApplicationDetail.tsx`
    - Remove `UnifiedLoader` imports, replace with `SkeletonCard` or `Skeleton` primitives
    - _Requirements: 1b.2, 1b.5_

- [x] 6. Replace UnifiedLoader imports — Admin pages and components
  - [x] 6.1 Migrate admin pages: `Dashboard.tsx`, `Applications.tsx`, `Settings.tsx`, `Programs.tsx`, `Intakes.tsx`, `AuditTrail.tsx`, `ProgramFees.tsx`
    - Remove `UnifiedLoader` imports, replace with `DashboardSkeleton` or inline `Skeleton` primitives
    - _Requirements: 1b.2, 1b.5_

  - [x] 6.2 Migrate admin components: `CommunicationHistory.tsx`, `UserActivityLog.tsx`, `EnhancedApplicationsManager.tsx`, `UserPermissions.tsx`, `EmailNotifications.tsx`, `ReportsGenerator.tsx`
    - Remove `UnifiedLoader`/`UnifiedSpinner` imports, replace with `Skeleton` primitives or `ButtonSpinner`
    - _Requirements: 1b.2, 1b.5_

  - [x] 6.3 Migrate admin application components: `ApplicationsTable.tsx`, `ApplicationDetailModal.tsx`, `ApplicationCard.tsx`, `ApplicationApprovalActions.tsx`, `StatusHistoryTab.tsx`, `GradesTab.tsx`, `DocumentsTab.tsx`
    - Remove `UnifiedLoader`/`UnifiedSpinner` imports, replace with `Skeleton` primitives or `ButtonSpinner`
    - _Requirements: 1b.2, 1b.5_

  - [x] 6.4 Migrate `App.tsx` Suspense fallback
    - Replace `UnifiedLoader` import with `getSkeletonFallback` usage
    - _Requirements: 1b.2, 13.4_

- [x] 7. Delete `UnifiedLoader.tsx` and verify zero references
...
- [x] 8. Checkpoint — Verify UnifiedLoader removal
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Focus indicator unification
  - [x] 9.1 Update `interactive-feedback.css` global focus rules
  ...
  - [x] 9.2 Replace legacy `focus:ring-blue-500` / `focus:ring-blue-600` patterns in admin components
...
- [x] 10. Color contrast and heading hierarchy fixes
  - [x] 10.1 Fix color contrast in `shape-landing-hero.tsx`
  ...
  - [x] 10.2 Fix heading hierarchy on Landing Page sections
  ...
  - [x] 10.3 Add dev-mode heading hierarchy validation to `PageShell.tsx`
  ...
  - [x] 10.4 Write property tests for contrast and heading utilities (Properties 2, 3)
...
- [x] 11. Enhance EmptyState and ErrorDisplay components
  - [x] 11.1 Add `secondaryAction` and `headingLevel` props to `EmptyState.tsx`
  ...
  - [x] 11.2 Add `onGoBack` and `supportUrl` props to `ErrorDisplay.tsx`
  ...
  - [x] 11.3 Write property tests for EmptyState and ErrorDisplay (Properties 5, 6)
...
- [x] 12. Checkpoint — Verify accessibility and component enhancements
  - Ensure all tests pass, ask the user if questions arise.

- [x] 13. Form accessibility, wizard fieldsets, and keyboard navigation
  - [x] 13.1 Add `<fieldset>`/`<legend>` wrappers to wizard step components
    - Wrap related fields in `BasicKycStep`, `EducationStep`, `PaymentStep`, `SubmitStep` with `<fieldset>` and descriptive `<legend>` elements
    - _Requirements: 5.1_

  - [x] 13.2 Add error summary with focus management to wizard
  ...
  - [x] 13.3 Write property test for form error aria-live (Property 4)
  ...
  - [x] 13.4 Write property tests for keyboard navigation (Properties 8, 9)
...
- [x] 14. Configuration and print stylesheet
  - [x] 14.1 Fix Tailwind dark mode config
    - Change `darkMode: 'media'` to `darkMode: 'class'` in `tailwind.config.js`
    - _Requirements: 14.3, 14.5_

  - [x] 14.2 Create print stylesheet and import it
  ...
  - [x] 14.3 Write property test for decorative aria-hidden (Property 10)
...
- [x] 15. Button loading state property test
  - [x] 15.1 Write property test for Button loading spinner (Property 1)
