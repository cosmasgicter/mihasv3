...
This bugfix addresses 8 issues discovered during a ScoutQA production readiness audit of the MIHAS-KATC Admissions Portal (***REMOVED***). The bugs span both the React frontend (Settings, Dashboard, SSE client, LandingPage, font loading) and the Django backend (sessions, token refresh, application tracking), covering UI state management defects, accessibility violations, SSE connection lifecycle gaps, image/font loading resilience, and backend endpoint reliability issues.
