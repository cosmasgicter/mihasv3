...
- **Fallback_Email_Constant**: The hardcoded `***REMOVED***` string used as a fallback alert email recipient across multiple backend files
...
1. THE Backend SHALL define the Fallback_Email_Constant (`***REMOVED***`) in exactly one shared location (audit item B8)
2. THE three files currently hardcoding `***REMOVED***` SHALL reference the shared constant instead of inline strings
