...
`settings.ERROR_ALERT_EMAIL` already exists in `backend/config/settings/base.py` (reads from `ERROR_ALERT_EMAIL` env var with fallback `ops@mihas.edu.zm`). The three files hardcoding `***REMOVED***` should use `settings.ERROR_ALERT_EMAIL` instead:
