...
1. THE ApiClient SHALL use `***REMOVED***/api/v1` as the base URL via the `VITE_API_BASE_URL` environment variable.
2. THE ApiClient SHALL NOT contain references to Vercel Functions, `api-src/`, or `lib/` backend utilities.
3. THE Admissions_App SHALL set `VITE_API_BASE_URL=***REMOVED***/api/v1` in its production environment configuration.
