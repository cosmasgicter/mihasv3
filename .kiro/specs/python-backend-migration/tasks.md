...
- [x] 9. Authentication system
  - [x] 9.1 Implement JWT authentication backend in `apps/accounts/authentication.py`
  ...
  - [x] 9.2 Implement JWT token generation/verification in `apps/accounts/tokens.py`
  ...
  - [x] 9.3 Implement password services in `apps/accounts/services.py`
  ...
  - [x] 9.4 Implement auth serializers in `apps/accounts/serializers.py`
    - Login, register, password reset request/confirm, session serializers
    - Input validation equivalent to current Zod schemas
    - _Requirements: 11.5_

  - [x] 9.5 Implement auth views in `apps/accounts/views.py`
  ...
  - [x] 9.6 Implement auth URL routing in `apps/accounts/urls.py`
    - Map all auth endpoints under `/api/v1/auth/`
    - _Requirements: 10.1_

  - [x]* 9.7 Write property tests for auth system
  ...
  - [x]* 9.8 Write property tests for auth cookies and cross-backend JWT
    - **Property 33: Auth cookie attributes**
    - **Validates: Requirements 18.2**
    - **Property 34: Shared JWT signing key — cross-backend token validity**
    - **Validates: Requirements 18.4**

- [x] 10. RBAC permission system
  - [x] 10.1 Implement permission classes in `apps/accounts/permissions.py`
...
- [x] 11. Checkpoint — Verify auth and RBAC
...
- [x] 12. Session management endpoints
  - [x] 12.1 Implement session views and URL routing
  ...
  - [x]* 12.2 Write property test for session revocation
    - **Property 32: Session revocation invalidates refresh token**
    - **Validates: Requirements 9.3**

- [x] 13. Application endpoints
  - [x] 13.1 Implement application serializers in `apps/applications/serializers.py`
  ...
  - [x] 13.2 Implement application filters in `apps/applications/filters.py`
    - Filter by status, payment, program, institution, search (name/email)
    - Sort by date, name, ASC/DESC
    - _Requirements: 4.3_

  - [x] 13.3 Implement application views in `apps/applications/views.py`
  ...
  - [x] 13.4 Implement application URL routing in `apps/applications/urls.py`
    - Map all application endpoints under `/api/v1/applications/`
    - _Requirements: 10.1_

  - [x]* 13.5 Write property tests for application CRUD
...
- [x] 14. Catalog endpoints
  - [x] 14.1 Implement catalog serializers in `apps/catalog/serializers.py`
  ...
  - [x] 14.2 Implement catalog views in `apps/catalog/views.py`
  ...
  - [x] 14.3 Implement catalog URL routing in `apps/catalog/urls.py`
    - Map all catalog endpoints under `/api/v1/catalog/`
    - _Requirements: 10.1_

  - [x]* 14.4 Write property tests for catalog
...
- [x] 15. Checkpoint — Verify core endpoints
...
- [x] 16. Document and payment endpoints
  - [x] 16.1 Implement file validators in `apps/documents/validators.py`
    - Magic byte verification + MIME type validation before storage
    - _Requirements: 6.1, 11.7_

  - [x] 16.2 Implement document serializers and views in `apps/documents/`
  ...
  - [x] 16.3 Implement payment views in `apps/documents/views.py`
  ...
  - [x] 16.4 Implement document/payment URL routing in `apps/documents/urls.py`
    - Map endpoints under `/api/v1/documents/` and `/api/v1/payments/`
    - _Requirements: 10.1_

  - [x]* 16.5 Write property test for file upload validation
    - **Property 18: File upload magic byte validation**
    - **Validates: Requirements 6.1, 6.2, 11.7**

  - [x]* 16.6 Write property test for input validation parity
    - **Property 25: Input validation parity**
    - **Validates: Requirements 11.5, 11.6**

- [x] 17. Celery async task system
  - [x] 17.1 Implement Celery app configuration in `config/celery.py`
    - Configure Celery with Redis broker, JSON serializer, autodiscover tasks
    - _Requirements: 12.1_

  - [x] 17.2 Implement email delivery task in `apps/common/tasks.py`
  ...
  - [x] 17.3 Implement OCR extraction task in `apps/documents/tasks.py`
  ...
  - [x]* 17.4 Write property tests for Celery tasks
...
- [x] 18. Checkpoint — Verify async task system
...
- [x] 19. Admin dashboard and user management endpoints
  - [x] 19.1 Implement admin views in `apps/accounts/views.py` (admin section)
  ...
  - [x] 19.2 Implement system settings CRUD
    - Key-value store with category, description, public/private visibility
    - _Requirements: 7.4_

  - [x] 19.3 Implement audit log query endpoint
    - Filter by entity type, action, actor, date range
    - _Requirements: 7.6_

  - [x]* 19.4 Write property tests for admin dashboard and settings
    - **Property 41: Dashboard counts consistency**
    - **Validates: Requirements 7.1**
    - **Property 42: Settings CRUD round-trip**
    - **Validates: Requirements 7.4**

- [x] 20. Notification and SSE endpoints
  - [x] 20.1 Implement notification views
  ...
  - [x] 20.2 Implement SSE view in `apps/common/sse.py`
    - Server-Sent Events for real-time updates with 8-second keepalive
    - Polling fallback endpoint
    - _Requirements: 9.4_

  - [x] 20.3 Implement email send endpoint
    - Admin-only endpoint to send email via Celery
    - Configure `RESEND_API_KEY` and `EMAIL_FROM` env vars
    - _Requirements: 8.3, 8.6_

  - [x]* 20.4 Write property test for notification preferences
    - **Property 29: Notification preference round-trip**
    - **Validates: Requirements 8.2**

- [x] 21. Idempotency and read-only mode
  - [x] 21.1 Implement idempotent request processing
  ...
  - [x] 21.2 Implement read-only mode
  ...
  - [x]* 21.3 Write property tests for idempotency and read-only mode
    - **Property 31: Idempotent request processing**
    - **Validates: Requirements 17.4**
    - **Property 38: Read-only mode blocks writes**
    - **Validates: Requirements 14.5**

- [x] 22. Checkpoint — Verify all endpoints and features
...
- [x] 23. Root URL configuration and API versioning
  - [x] 23.1 Wire all app URL configs into `config/urls.py`
  ...
  - [x]* 23.2 Write property test for API versioning prefix
    - **Property 20: API versioning prefix**
    - **Validates: Requirements 10.1**

- [x] 24. Contract testing and migration toolkit
  - [x] 24.1 Create contract test recording fixtures
  ...
  - [x] 24.2 Implement contract parity test suite
  ...
  - [x]* 24.3 Write property test for contract response parity
    - **Property 35: Contract response parity**
    - **Validates: Requirements 19.2, 19.3**

  - [x] 24.4 Implement migration verification scripts
  ...
  - [x]* 24.5 Write property test for migration idempotency
...
- [x] 25. Deployment configuration
  ...
  - [x] 25.2 Configure database connection pooling
  ...
  - [x] 25.3 Configure WhiteNoise for static file serving
