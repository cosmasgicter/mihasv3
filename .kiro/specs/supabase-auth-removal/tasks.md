...
- [ ] 1. Fix deployment blockers in vercel.json
...
- [ ] 2. Create custom auth types
  - [ ] 2.1 Create src/types/auth.ts with User, UserProfile, AuthSession, SignInResult, SignUpResult interfaces
...
- [ ] 3. Update AuthContext to use custom types
  - [ ] 3.1 Update src/contexts/AuthContext.tsx
...
- [ ] 4. Update auth hooks to use custom types
  - [ ] 4.1 Update src/hooks/auth/useOptimizedAuthState.ts
    - Remove local User interface definition (lines ~30-35)
    - Import User from @/types/auth
    - _Requirements: 2.2_
  
  - [ ] 4.2 Update src/hooks/auth/useRoleQuery.ts
...
- [ ] 5. Clean up src/lib/supabase.ts
  - [ ] 5.1 Update src/lib/supabase.ts
...
- [ ] 6. Update analytics and analysis files
  - [ ] 6.1 Update src/lib/analytics.ts
    - Remove `import type { Session } from '@supabase/supabase-js'`
    - Replace with inline type or remove if unused
    - _Requirements: 2.2_
  
  - [ ] 6.2 Update src/analysis/security/SecurityAnalyzer.ts
    - Configure Supabase client with auth disabled
    - _Requirements: 6.1_
  
  - [ ] 6.3 Update src/analysis/security/RLSPolicyAnalyzer.ts
    - Configure Supabase client with auth disabled
    - _Requirements: 6.2_
  
  - [ ] 6.4 Update src/analysis/security/FunctionSearchPathAnalyzer.ts
    - Configure Supabase client with auth disabled
    - _Requirements: 6.3_
  
  - [ ] 6.5 Update src/analysis/database/SchemaAnalyzer.ts
    - Configure Supabase client with auth disabled
    - _Requirements: 6.4_

- [ ] 7. Checkpoint - Verify type changes compile
  - Run `bun run type-check` to verify no TypeScript errors
  - Fix any remaining type issues before proceeding
  - _Requirements: 7.2_

- [ ] 8. Remove deprecated Supabase Auth UI components
  - [ ] 8.1 Delete src/components/supabase-ui/auth-form.tsx
    - File is marked @deprecated and not imported anywhere
    - _Requirements: 3.3_
  
  - [ ] 8.2 Update src/components/supabase-ui/index.ts
...
- [ ] 9. Remove Supabase Auth UI packages from dependencies
  ...
  - [ ] 9.2 Run bun install to update lockfile
    - Verify packages are removed from bun.lock
    - _Requirements: 3.1, 3.2_

- [ ] 10. Checkpoint - Verify build succeeds
