...
**`views.py:PasswordResetRequestView.post()`** — Creates an `EmailQueue` record with a password reset link (`***REMOVED***/auth/reset-password?token={raw_token}`), then dispatches `send_email_task.delay(email_queue_id)`.
...
def check_uptime_task(self):
    ...
    # GET configured HEALTH_CHECK_URL (default: ***REMOVED***)
    # 10-second timeout
...
| `HEALTH_CHECK_URL` | `backend/.env.example` | `***REMOVED***` |
...
*For any* generated password reset token, the `EmailQueue` record body created for the reset email must contain both the raw token string and the frontend base URL (`***REMOVED***`).
