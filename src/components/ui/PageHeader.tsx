...
export function PageHeader({
  ...
  const renderStat = (stat: PageHeaderStat, index: number) => {
    ...
    const baseClasses = isGradient
      ? 'bg-white dark:bg-gray-800/15 border-white/30 text-white'
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <p className={cn('text-xs font-semibold uppercase tracking-wide', isGradient ? 'text-white/80' : 'text-gray-500 dark:text-gray-500')}>
            {label}
          </p>
            ...
            {icon && (
              <div
                className={cn(
                  ...
                  isGradient ? 'border-white/30 bg-white dark:bg-gray-800/15 text-white' : 'border-blue-600/10 bg-primary/5 text-blue-600'
                )}
              >
                {icon}
              </div>
