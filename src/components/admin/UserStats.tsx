...
export function UserStats({ users, className = '' }: UserStatsProps) {
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      ...
      default:
        return 'bg-accent text-gray-900 border-border'
    }
  }
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        ...
        <div className="space-y-3">
          {recentUsers.length > 0 ? (
              ...
              <div key={user.user_id} className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-muted">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{sanitizeForDisplay(user.email)}</p>
                  </div>
                </div>
            ...
            <p className="text-gray-900 text-center py-4">No recent users</p>
          )}
        </div>
      </div>
    </div>
