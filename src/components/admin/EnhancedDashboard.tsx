      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        >
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <h3 className="text-lg font-bold text-foreground flex items-center gap-2"><TrendingUp className="w-5 h-5 text-primary" /> Recent Activity</h3>
            <Button
              variant="ghost"
              size="sm"
              onClick={onRefresh}
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-96 overflow-y-auto">
            {recentActivity.length > 0 ? recentActivity.map((activity, index) => (
              <div key={activity.id || index} className="flex items-start space-x-3 p-3 bg-muted/50 rounded-lg hover:bg-primary/5 transition-colors"
              >
