...
export class GradeConverter {
  ...
  /**
   * Converts percentage to Zambian grade (approximate)
   */
  static percentageToGrade(percentage: number): number {
    if (percentage >= 90) return 1  // A+
    if (percentage >= 80) return 2  // A
    if (percentage >= 75) return 3  // B+
    if (percentage >= 70) return 4  // B
    if (percentage >= 65) return 5  // C+
    if (percentage >= 50) return 6  // C
    if (percentage >= 40) return 7  // D
    if (percentage >= 35) return 8  // E
    return 9  // F
  }

  /**
   * Converts GPA (4.0 scale) to approximate Zambian grade
   */
  static gpaToGrade(gpa: number): number {
    if (gpa >= 3.7) return 1  // A+
    if (gpa >= 3.3) return 2  // A
    if (gpa >= 3.0) return 3  // B+
    if (gpa >= 2.7) return 4  // B
    if (gpa >= 2.3) return 5  // C+
    if (gpa >= 2.0) return 6  // C
    if (gpa >= 1.7) return 7  // D
    if (gpa >= 1.0) return 8  // E
    return 9  // F
  }

  /**
   * Converts international letter grades to Zambian numeric
   */
  static internationalLetterToGrade(letter: string): number | null {
    const normalized = letter.toUpperCase().trim()
    
    // Handle A+, A, A- format
    if (normalized.includes('A')) {
      if (normalized.includes('+')) return 1  // A+
      if (normalized.includes('-')) return 3  // A- -> B+
      return 2  // A
    }
    
    if (normalized.includes('B')) {
      if (normalized.includes('+')) return 3  // B+
      if (normalized.includes('-')) return 5  // B- -> C+
      return 4  // B
    }
    
    if (normalized.includes('C')) {
      if (normalized.includes('+')) return 5  // C+
      if (normalized.includes('-')) return 7  // C- -> D
      return 6  // C
    }
    
    if (normalized.includes('D')) return 7  // D
    if (normalized.includes('F')) return 9  // F
