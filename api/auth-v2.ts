 ...
 * 
 * Endpoints:
 * - POST /api/auth-v2?action=login
 * - POST /api/auth-v2?action=logout
 * - POST /api/auth-v2?action=refresh
 * - GET /api/auth-v2?action=session
 */
...
async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    console.error("[AUTH-V2] Unhandled error:", error);
...
async function handleLogin(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    if (!passwordValid) {
      ...
      console.log(`[AUTH-V2] Failed login attempt: ${email}`);
      return sendError(res, "Invalid credentials", HttpStatus.UNAUTHORIZED);
    }
...
async function handleLogout(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    if (user) {
      ...
      console.log(`[AUTH-V2] User logged out: ${user.userId}`);
    }
...
async function handleSession(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    console.error("[AUTH-V2] Session error:", error);
...
async function handleRegister(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    console.log(`[AUTH-V2] User registered: ${newUser.id} by ${currentUser.userId}`);
