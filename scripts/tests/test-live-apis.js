...
/**
 * Live API Testing Script for MIHAS Application System
 * Tests all API endpoints on the live Netlify deployment
 */
...
const tests = {
  ...
  getPrograms: async () => {
    ...
    return {
      ...
      details: isSuccess ? { count, sample: response.data?.slice(0, 2) } : response.data
    };
  },
  ...
  getAdminApplications: async () => {
    ...
    return {
      ...
      details: isSuccess ? { count, sample: response.data?.slice(0, 3) } : response.data
    };
  },
...
const runAllTests = async () => {
  log('🚀 Starting Live MIHAS API Testing...', 'info');
  ...
  const reportPath = './live-api-test-results.json';
