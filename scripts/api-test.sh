...
# Tests all endpoints against production: https://apply.mihas.edu.zm

BASE="https://apply.mihas.edu.zm"
...
get() {
  curl -s --max-time 15 \
    -H "User-Agent: ${UA}" \
    -H "Origin: https://apply.mihas.edu.zm" \
    "$1"
}
...
LOGIN_R=$(curl -s --max-time 15 \
  ...
  -H "Content-Type: application/json" \
  -H "Origin: https://apply.mihas.edu.zm" \
  -d '{"email":"admin@mihas.edu.zm","password":"Admin123!"}' \
