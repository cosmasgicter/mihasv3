...
**Tag:** confirmed-bug
**Issue:** Contains live Lenco public key (`***REMOVED***`) and uses GlitchTip project 22431. This is the production `.env` that gets loaded by Vite. The `VITE_APP_VERSION=45d5ee7e7` looks like a commit SHA fragment — correct for production but inconsistent with other env files showing `1.0.0`.
...
**Tag:** confirmed-bug
**Issue:** 1) Has an API rewrite rule (`/api/:path*` → `***REMOVED***/api/:path*`) which contradicts the steering doc stating "Vercel free tier does not support external rewrites." This rewrite will silently fail on Vercel Hobby plan, causing API calls to 404. 2) CSP `connect-src` is missing Lenco domains that may be needed if jobs-ops ever handles payments. 3) The `X-CSP-Note` header leaks internal security planning to the public.
**Location:** rewrites section (line 7-10), headers section
**Recommendation:** Remove the `/api/:path*` rewrite — jobs-ops should call `***REMOVED***` directly (cross-origin with credentials), matching the admissions pattern. Remove the `X-CSP-Note` header from production.
...
**Tag:** suspicious-stale-path
**Issue:** Uses `axios` and points to `https://mihasv3.pages.dev` as the base URL — this is a stale Cloudflare Pages URL. The current API is at `***REMOVED***`. Also uses Bearer token auth instead of cookie-based auth.
**Location:** Line 12 (baseUrl), Line 14 (Authorization header)
**Recommendation:** Update base URL to `***REMOVED***`. Update auth pattern to match current cookie-based auth. Or remove if this example is no longer relevant.
