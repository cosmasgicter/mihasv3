...
RESPONSE=$(curl -s -X POST ***REMOVED***/auth/login \
...
DASH_RESPONSE=$(curl -s ***REMOVED***/admin/dashboard \
...
USERS_RESPONSE=$(curl -s ***REMOVED***/admin/users \
  -H "Authorization: Bearer $TOKEN")
...
PROGRAMS_RESPONSE=$(curl -s ***REMOVED***/catalog/programs)
