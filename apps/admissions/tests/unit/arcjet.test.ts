...
// Simulate handleArcjetDecision behavior
function simulateHandleArcjetDecision(decision: { isDenied: () => boolean }, res: ReturnType<typeof createMockResponse>) {
  if (decision.isDenied()) {
    res.status(403).json({ success: false, error: 'Request blocked by security policy', code: 'SECURITY_VIOLATION' });
    return true;
  }
  return false;
}

describe('Arcjet Integration - Rate Limit Configurations (Requirement 2.7)', () => {
...
describe('Arcjet Integration - Blocked Request Responses (Requirement 2.5)', () => {
  ...
  it('should return consistent response for all block types', () => {
...
describe('Arcjet Integration - Service Unavailable Response (Requirement 2.8)', () => {
  ...
  it('should fail secure by blocking requests when service unavailable', () => {
    const expectedBehavior = { allowRequest: false, statusCode: 503, errorCode: 'SECURITY_SERVICE_ERROR' };
...
describe('Arcjet Integration - Response Structure Consistency', () => {
...
describe('Arcjet Integration - Security Requirements', () => {
