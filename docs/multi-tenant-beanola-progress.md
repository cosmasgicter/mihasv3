...
- Task 23.1 — expanded the admin Tenant Onboarding page (`apps/admissions/src/pages/admin/Tenants.tsx`) to the full IA and resolved its two pre-existing restricted-import lint errors (`@/components/ui/card`, `@/components/ui/badge` → canonical `SectionCard`/`StatusBadge`/`MetricTile` barrel primitives):
  ...
  - Structured rule builders replace raw JSON entry: per-offering assignment priority + offering status, and allow/deny country chip lists (`TokenChips`) persisted to `programs.assignment_rules` through the tenant-scoped admin program endpoint.
  ...
  - New frontend service reads/writes: `listOfferings`, `updateOfferingRules`, `listSettlements` in `services/admin/tenants.ts`.
