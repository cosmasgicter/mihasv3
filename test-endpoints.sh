#!/bin/bash
TOKEN="eyJhbGciOiJIUzI1NiIsImtpZCI6IjE1ZTkxenVweDltUlBkU00iLCJ0eXAiOiJKV1QifQ..S2MeiZpEsshOV5025a4QuXiCj4IKJq7VKEAsm0yTVpY"

echo "=== /api/auth-roles ==="
curl -s -w "\nStatus: %{http_code}\n" "***REMOVED***/api/auth-roles" -H "Authorization: Bearer $TOKEN"

echo -e "\n=== /api/notifications ==="
curl -s -w "\nStatus: %{http_code}\n" "***REMOVED***/api/notifications" -H "Authorization: Bearer $TOKEN" | head -10

echo -e "\n=== /applications (draft) ==="
curl -s -w "\nStatus: %{http_code}\n" "***REMOVED***/applications?page=0&pageSize=1&status=draft&mine=true" -H "Authorization: Bearer $TOKEN"

echo -e "\n=== /applications/[id] ==="
curl -s -w "\nStatus: %{http_code}\n" "***REMOVED***/applications/3eb2a6b8-8e93-4e0c-a8b7-73537615b87e" -H "Authorization: Bearer $TOKEN" | head -5
